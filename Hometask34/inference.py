...
def get_model(checkpoint):
    """
    Loads fine-tuned Mask2Former model from `checkpoint` for panoptic segmentation.
    
    Returns a pair `(processor, model)`.
    """
...
def draw_image(image, model, segmentation, segments_info):
    """
...
def panoptic_segmentation(input_image):
    ...
    fig = draw_image(input_image, model, **results)
    buf = io.BytesIO()
